...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/login', () => {
    ...
    test('should return 401 error if there are no users with that email', async () => {
      ...
      expect(res.body).toEqual({ code: httpStatus.UNAUTHORIZED, message: 'Email ou mot de passe incorrect' });
    });
  ...
  describe('POST /v1/auth/forgot-password', () => {
    ...
    test('should return 204 and send reset password email to the user', async () => {
      ...
      expect(sendResetPasswordEmailSpy).toHaveBeenCalledWith(userOne.email, expect.any(String), 'http://fakeOrigin.fake');
...
describe('Auth middleware', () => {
  ...
  test('should call next with unauthorized error if access token is not found in header', async () => {
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({ statusCode: httpStatus.UNAUTHORIZED, message: 'Veuillez vous authentifier' })
    );
  });
  ...
  test('should call next with forbidden error if user does not have required rights and userId is not in params', async () => {
    ...
    expect(next).toHaveBeenCalledWith(expect.any(ApiError));
    expect(next).toHaveBeenCalledWith(expect.objectContaining({ statusCode: httpStatus.FORBIDDEN, message: 'Interdit' }));
  });
